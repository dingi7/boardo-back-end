...
cardController.post('/cards', async (c: Context) => {
    ...
    if (!reqBody.content || !reqBody.listId || !user?._id || !reqBody.organizationId) {
        return c.json({ error: 'Missing required fields' }, 400);
    }
    const content = reqBody['content'].toString();
    const listId = reqBody['listId'].toString();
    const result = await createCard(content, listId, user._id, reqBody.organizationId);
...
cardController.delete('/cards/:cardId', async (c: Context) => {
    ...
    const user = checkAuthorization(c);
    if (!cardId || !user?._id || !reqBody.boardId || !reqBody.organizationId) {
        return c.json(
    ...
    const result = await deleteCardById(cardId, user!._id, reqBody.organizationId);
